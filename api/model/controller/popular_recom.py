...
class PopularRecom():
    ...
    def recommend(self):
        pop_api = 'http://10.35.2.45/product-tag/api/recommend/popular/?theme=%E6%B8%AC%E8%A9%A6&use_type=reco.system&sub_use_type=organic&recommend_product_type=grp&use_utm=0&rate=0'
        response = requests.get(pop_api).json()

        pop_grp = list()
        TOP = 5
        for grp_info in self.get_grp_info(response['Data']['results'][0]['json_content']):
            pop_grp.append(grp_info)
            if len(pop_grp) == TOP:
                break
